...
class Checker(Tester):
    def __init__(self, **kwargs):
        kwargs.setdefault("reference_command", 'systemctl list-units')
        kwargs.setdefault("reference_command_wrong", 'systemctl list-units --type=service')
    ...
    def reference_command_wrong(self):
        ...
        if reference_output_wrong_command == self.stdout:
            return self.fail("Нужно вывести все службы, а не только сервисы")
